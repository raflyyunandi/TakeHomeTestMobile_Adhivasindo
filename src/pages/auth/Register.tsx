...
import { registerUser, fetchMe, setTokenHeader } from '../../lib/api'
...
export default function Register() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      setError(e?.response?.data?.message || 'Gagal daftar')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div className="w-full max-w-[420px] rounded-card border border-border bg-panel p-4 shadow-panel">
            <div className="mb-3 flex items-center gap-3 border-b border-border pb-3">
              ...
              <div>
                ...
                <div className="mt-0.5 text-xs text-muted">Daftar User</div>
              </div>
            </div>
            <label className="mb-1 block text-xs font-bold text-slate-700">Nama</label>
            <IonInput className="mb-3 rounded-xl border border-border bg-white px-3" value={name} onIonChange={(e) => setName(e.detail.value || '')} />
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <IonButton className="flex-1 rounded-xl bg-brand font-extrabold" onClick={handleRegister}>Daftar</IonButton>
              <IonButton className="flex-1 rounded-xl border border-border bg-white font-extrabold text-slate-800" onClick={() => history.push('/login-user')}>Login</IonButton>
            </div>
          </div>
        </div>
